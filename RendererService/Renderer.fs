...
let triangle (pts : List<Vector<double>>) (width : int) (height : int)
    ...
    let mutable bboxmax = vector [ 0.; 0. ]
    let clamp = vector [double width - 1.; double height - 1.]
    List.iter (fun (pt: Vector<double>) ->
        for i in 0..1 do
          bboxmin.[i] <- min bboxmin.[i] pt.[i] |> max 0.
          bboxmax.[i] <- max bboxmax.[i] pt.[i] |>  min clamp.[i] 
    ) pts
    ...
    for x = xMin to xMax do
      for y = yMin to yMax do
        let bcScreen = barycentric pts.[0] pts.[1] pts.[2] (vector [double x; double y; 0.])
